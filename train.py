...
class RearrangeBalls(object):
    ...
    def __call__(self, img):
        ...
        return torch.cat((img[:, [0]],
                          img[:, [1], idx1].unsqueeze(1),
                          img[:, [2]],
                          img[:, [3], idx2].unsqueeze(1),), 1)


class RearrangeItems(object):
    def __call__(self, img):
        # img is steps * row * column * 3 * 28 * 28
        indices = [[[(r, c) for r in range(i*3, i*3+3) for c in range(j*3, j*3+3)]
                    for j in range(2)]
                   for i in range(2)]
        for i in range(2):
            for j in range(2):
...
def get_gridworld_datasets(img_pth, label_pth, action_pth, transform, train_frac, device):
...
def run(epoch, cv_model, domain_model, optimizer, data_loader, gamma, lambda_, to_train=False):
    ...
    for i, (data, label, action) in enumerate(data_loader):
        ...
        with torch.set_grad_enabled(to_train):
            ...
            loss += F.mse_loss(validity_constraint,
                            torch.zeros(validity_constraint.shape,
                                        dtype=validity_constraint.dtype,
                                        device=device),
                            reduction='sum')
            # Add a prior
            loss += lambda_*F.mse_loss(precon,
                                torch.ones(precon.shape,
                                            dtype=precon.dtype,
                                            device = device),
                                reduction='sum')
            if to_train:
        ...
        acc_final += computeCorrectness(preds.data, label[:, :-1].flatten(start_dim = 0, end_dim = 1))

    if to_train:
        print('Epoch {} TRAINING SET RESULTS: Average loss: {:.4f} Acc: {:.4f}'.format(epoch, loss_final, acc_final/len(trainset)))
    else:
        print('Epoch {} TESTING SET RESULTS: Average loss: {:.4f} Acc: {:.4f}'.format(epoch, loss_final, acc_final/len(testset)))
