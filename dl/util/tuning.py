...
def nn_task(network, path, train_dataset, val_dataset, parameters):
    # Print the class precedence list (method resolution order)
    print("Class precedence list:")
    for c in network.mro():
        print(" ", c)

    resume_hash = parameters.pop("resume_hash", None)
    if not resume_hash:
        # Initialize the network
        net = network(path, parameters=parameters)
        # Train the network
        net.train(train_dataset, val_dataset)
    else:
        net = network(os.path.join(path, "logs", resume_hash), parameters={})
        net.resume(train_dataset, val_dataset, parameters)

    error = np.array(list(net.evaluate(val_dataset, batch_size=100).values()))
...
def update_history(path, key, new_entry):
    history_path = os.path.join(path, "grid_search.log")
    history = []
        ...
        with open(history_path, 'r') as f:
            for line in f:
                try:
                    ...
                    print("Error loading a line from the history:", e)

    found = False
    # Look for an entry with the same configuration key.
    for i, entry in enumerate(history):
        if _key(entry[1]) == key:
            ...
            break
    if not found:
        # If no matching entry is found, append the new entry.
        history.append(new_entry)

    # Write the updated history back to the file.
    with open(history_path, 'w') as f:
        for entry in history:
            json.dump(entry, f, cls=NpEncoder)
            f.write("\n")
    return load_history(path)
...
################################################################
# algorithms
keys_to_ignore = ["time_start", "time_duration", "time_end", "HOSTNAME", "LSB_JOBID", "gpu", "hash"]
...
def simple_genetic_search(task, parameters, path,
                          ...
                          population=10,
                          limit=float('inf'),
                          report=None, report_best=None, resume_mode=False):
    ...
    if not (initial_population <= limit):
        print({"initial_population": initial_population}, "is superceded by", {"limit": limit},
              ". limit must be larger than equal to the initial population", )
        initial_population = limit

    # assert population <= initial_population
    if not (population <= initial_population):
        print({"population": population}, "is superceded by", {"initial_population": initial_population},
              ". initial_population must be larger than equal to the population", )
    ...
    def _iter(config):
        ...
        def fn1():
            open_list, close_list = load_history(path)
            if not resume_mode and _key(config) in close_list:
                raise HyperparameterGenerationError()
            else:
                # For both normal and resume mode, insert a placeholder.
                save_history(path, (float("inf"), config, "placeholder"))
        ...
        # In resume mode, update the existing history entry;
        # otherwise, simply append a new log entry.
        if resume_mode:
            open_list, close_list = call_with_lock(
                path, lambda: update_history(path, _key(config), (eval, config, None))
            )
        else:
            open_list, close_list = call_with_lock(
                path, lambda: save_history(path, (eval, config, None))
            )
        ...
        while _runs(open_list) <= initial_population:
            ...
            for _, config in zip(range(max_trials), gen_config):
                try:
                    ...
                    open_list, close_list = call_with_lock(path, lambda: save_history(path, (
                    float("inf"), config, "invalid")))
